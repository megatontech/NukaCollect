...
namespace NukaCollect.Reports
{
    ...
    public partial class TimePeriodReportBase : ReportBase
    {
        ...
        private class TimePeriodDesignExtention : ReportDesignExtension
        {
        ...
        private class TimePeriodParametersExtention : ParametersRequestExtension
        {
            private bool canUpdateEditors = true;

            private static EditParameterInfo FindParameterInfo(IList<EditParameterInfo> parametersInfo, string name)
            {
                return new List<EditParameterInfo>(parametersInfo).Find(delegate (EditParameterInfo item) { return item.Parameter.Name == name; });
            }

            private static BaseEdit FindEditor(IList<EditParameterInfo> parametersInfo, string name)
            {
            ...
            private static BaseEdit CreatePeriodEditor(Array values, string[] names)
            {
            ...
            private void OnStartDateDateEditValueChanged(IList<EditParameterInfo> parametersInfo)
            {
                if (!canUpdateEditors)
                    return;
            ...
            private void OnEndDateEditValueChanged(IList<EditParameterInfo> parametersInfo)
            {
                if (!canUpdateEditors)
                    return;
            ...
            private void OnPeriodEditValueChanged(IList<EditParameterInfo> parametersInfo)
            {
                if (!canUpdateEditors)
                    return;
            ...
            private void SetValue(BaseEdit editor, object value)
            {
        ...
        private static TimePeriod defaultTimePeriod = TimePeriod.ThreeMonths;
        ...
        private static DateTimeCorrector CreateCorrector(TimePeriod timePeriod)
        {
            switch (timePeriod)
            {
        ...
        private Parameter CreateParameter(string name, Type type, string description, object value)
        {
