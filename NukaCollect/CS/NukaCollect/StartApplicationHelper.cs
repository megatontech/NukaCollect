...
namespace DevExpress.DemoData.Helpers
{
    ...
    {
        private static EscapeHelper urlHelper;
...
#if !SL
        private static string commonDocuments;
        ...
        private static void StartCore(string fileName, string workingDirectory, string argumentsString)
        {
        ...
        private static bool StartWithRegistryInfo(string fileName, string workingDirectory, string argumentsString)
        {
            try
            {
        ...
        private static bool StartWithVSPath(string fileName, string workingDirectory, string argumentsString)
        {
            try
            {
        ...
        private static void DoStart(string fileName, string workingDirectory, string argumentsString)
        {
        ...
        private class VSInstallHelper
        {
            ...
            private static string GetVS2010Path()
            {
                if (vs2010Path == null)
            ...
            private static string GetVS2012Path()
            {
                if (vs2012Path == null)
            ...
            private static string GetVSPathCore(string version)
            {
                try
                {
        ...
        private class VSSolutionFile
        {
            ...
            private string GetVersionInLine(string line)
            {
    ...
    {
        private List<Dictionary<string, string>> replaces = new List<Dictionary<string, string>>();
        private List<Dictionary<string, string>> inverts = new List<Dictionary<string, string>>();
        ...
        private static void AddReplace(string from, string to, List<Dictionary<string, string>> replaces)
        {
            Dictionary<string, string> replace = GetDictionary(replaces, from.Length - 1);
            replace.Add(from, to);
        }

        private static string Replace(string s, List<Dictionary<string, string>> replaces)
        {
        ...
        private static Dictionary<string, string> GetDictionary(List<Dictionary<string, string>> list, int i)
        {
            lock (list)
            {
    ...
    public abstract class DisposableBase : IDisposable
    {
        ...
        private void RaiseAfterDispose()
        {
    ...
    public class DbEngineDetector
    {
        ...
        private static string PatchInnerString(string connectionString, bool entitySyntax, Func<string, string> f)
        {
            if (entitySyntax)
            {
